    ...
    def post(self, request):
        try:
            ...
            existing_rtqm_mt = RtqmMt.objects.filter(
                buyer_code=rtqm_mt_data['buyer'],
                style=rtqm_mt_data['styleno'],
                ourref=rtqm_mt_data['ourref'],
                color=rtqm_mt_data['color']
            ).first()
            ...
            if existing_rtqm_mt:
                ...
                print("data pahle se mojood hai")
            else:
                rtqm_no = get_next_number('RTQM')
                rtqm_mt = RtqmMt.objects.create(
                    buyer_code=rtqm_mt_data['buyer'],
                    rtqm_no =rtqm_no,
                    buyer_name=rtqm_mt_data['buyer_name'],
                    style=rtqm_mt_data['styleno'],
                    ourref=rtqm_mt_data['ourref'],
                    color=rtqm_mt_data['color'],
                    total_quantity=quantity,
                    unit_id=rtqm_mt_data['unit_id'],
                    floor_id=rtqm_mt_data['floor_id'],
                    line_id=rtqm_mt_data['line_id']
                )
        ...
        try:
            ...
            with transaction.atomic():
                ...
                for item in pass_data:
                    ...
                    # Create RtqmCounter objects based on item_quantity
...
class RtqmMtDtRectifiedView(View):
    ...
    def get_defect_dt(self, request):
        ...
        rtqm_mt_queryset = RtqmMt.objects.filter(buyer_code=buyer_code, style=style, color=color)
        # print("rtqm_mt_queryset",rtqm_mt_queryset)
        if rtqm_mt_queryset.exists():
            rtqm_counter_queryset = RtqmCounter.objects.filter(
                rtqm_mt__buyer_code=buyer_code,
                status='DEFECT',
    ...
    def post(self, request):
        try:
            ...
            if status == 'REJECT':
                ...
                rtqm_counter.status = 'REJECT'
            elif status == 'RECTIFIED':
                rtqm_counter.rectified = 'RECTIFIED'
                rtqm_counter.status = 'RFT'
            elif status == 'DEFECT':
