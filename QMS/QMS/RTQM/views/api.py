...
class RtqmAPI(View):
    ...
    def counter_rtqm_pdrr(self, id):
        ...
        defect_count = RtqmCounter.objects.filter(
            rtqm_mt=rtqm_mt, 
            status='DEFECT'
        ).filter(
            Q(rectified=None) | Q(rectified='DEFECT')
        ).count()
        reject_count = RtqmCounter.objects.filter(
            rtqm_mt=rtqm_mt,
            rectified='REJECT',
            ).filter(
                 Q(status='REJECT') | Q(status='DEFECT')
                ).count()
    ...
    def today_counter_pdrr(self,id):
        ...
        rectified_count = RtqmCounter.objects.filter(rtqm_mt=rtqm_mt,updated_at__date=today_date, rectified='RECTIFIED').count()
