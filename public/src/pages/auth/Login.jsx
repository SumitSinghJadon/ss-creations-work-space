...
const Login = () => {
  ...
  //   axios.get(`${DjangoConfig.apiUrl}/rtqm/get-unit/`).then((response) => {
  //     console.log("Unit Data",response.data[0].id)
  //     const data = {
  //       unit_id: response.data[0].id,
  //     };
  //     const queryParams = new URLSearchParams(data).toString();
  //     axios.get(`${DjangoConfig.apiUrl}/rtqm/get-line/?${queryParams}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    try {
      const userData1 = lineData.find((key) => key.id === parseInt(lineId));
      if (userData1) {
        if ( userData1.value === password ) {
