...
import random
from IS_Nexus.templatetags.template_filter import get_buyer_name
...
class SilhouettesView(View):
    def get_all_silhouettes(self,request):
        silhouettes_data =StyleSilhouettes.objects.all().values()
        list_silhouettes = list(silhouettes_data)
        
        for silhouette in list_silhouettes:
            buyer_code = silhouette['buyer']
            silhouette['buyer'] = get_buyer_name(buyer_code)
        
        return list_silhouettes
        # return list_silhouettes
    def get(self,request):
        silhouettes_data = self.get_all_silhouettes(request)
        context ={
            'silhouettes_data':silhouettes_data
        }
