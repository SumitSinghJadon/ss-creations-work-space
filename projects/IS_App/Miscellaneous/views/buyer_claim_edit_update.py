...
class BuyerClaimEditUpdate(View):
    def get(self, request,id):
        ...
        data = BuyerClaimModel.objects.get(id=id)
        flag = request.GET.get('flag')
        mid = request.GET.get('financial_year')
        inv = request.GET.get('invoice_no')

        print(f"Flag: {flag}, Financial Year: {mid}, Invoice No: {inv}")
        ...
        if flag == 'get_invoice_by_year':
            ...
            if mid and inv:
                sql_query = f"""
               SELECT DISTINCT i.InvHomeLocation,i.HomeLocation,i.Buyer,i.Currency, d.styleno, p.party_name,p.party_name
        ...
        return render(request, 'buyer_claim_entry_edit_update.html', context)
    
    def post(self,request,id):
        data = BuyerClaimModel.objects.get(id=id)
        if data:
            data.financial_year = request.POST.get('financial_year')
            data.loss_type = request.POST.get('type')
            loss_date = request.POST.get('claim_date')
            changed_claim_date = datetime.strptime(loss_date, "%d-%m-%Y").strftime("%Y-%m-%d")
            data.loss_date = changed_claim_date
            data.invoice_no = request.POST.get('invoice_no')
            data.buyer = request.POST.get('buyer_code')
            data.style = request.POST.get('styles')
            data.unit_name = request.POST.get('unit_code')
            data.amount_fc = request.POST.get('amount_fc')
            data.currency = request.POST.get('currency')
            data.conversion_rate = request.POST.get('conversion_rate')
            data.amount = request.POST.get('amount')
            data.debit_note_amount = request.POST.get('debit_note_amount')
            data.debit_no = request.POST.get('debit_no')
            data.reason = request.POST.get('reason')
            data.remarks = request.POST.get('remarks')
            data.save()
            return redirect('buyerClaimDetails')
