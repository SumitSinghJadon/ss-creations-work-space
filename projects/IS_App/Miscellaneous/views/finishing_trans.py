...
class FinishingReportView(View):
    def get(self,request):
        ...
        if unit_code:
            ...
            sql = f"""  EXEC GET_CUTTING_DATA_ERP '{dayno}','{unit_code_erp}','finishing' """
            cursor.execute(sql)
            ...
            cursor = connections['default'].cursor()
            sql = f"""  EXEC GET_DAILY_MANP_LIST_DEPT_COST '{month}','{year}','{unit_code}',12,'one_day_total_cost','{dayno}' """
            # print(sql)
            cursor.execute(sql)
            ...
            cursor = connections['default'].cursor()
            sql = f"""  EXEC GET_CUTTING_DATA_ERP '{dayno}','{unit_code_erp}','finishing_total' """
            cursor.execute(sql)
        ...
        return render(request,'finishing_trans.html',context)
