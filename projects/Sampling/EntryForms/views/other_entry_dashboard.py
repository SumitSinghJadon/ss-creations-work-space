...
class OtherDashboardView(View):
    def get(self, request):
        ...
        (case WHEN c.other_type ='1' THEN 'Fresh' WHEN c.other_type ='2' THEN 'Alter' WHEN c.other_type ='3' THEN 'Recut' end) other_type,
        c.size_breakup as other_size,c.qty,
        t.emp_name as cutter_name,c.assign_date as other_date 
        from sample_booking_mt (nolock) s join other_entry_mt (nolock) c on s.id = c.booking_id 
        ...
        other_trans_list =  [{cursor.description[i][0]: value for i, value in enumerate(row)} for row in cursor.fetchall()]

        context = {
            'other_trans_list' : other_trans_list,
        }
        return render(request, 'other_entry_dashboard.html', context)
