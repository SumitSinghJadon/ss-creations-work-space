...
from Sampling_db.models import FinishEntryMt, FinishTransactionDt
from django.contrib import messages
from Sampling_db.models import SampleBookingMt,SampleArticleDetails,SampleSizeQuantity , StitchTransactionDt
...
from functions import delete_unused_finish_temp_trans , samp_booking_data , cutting_size_color_wise_qty , qa_name_list, tailor_name_list



class FinishingTransactionEntryView(View):
    def get(self, request):
        booking_no              = StitchTransactionDt.objects.filter(stitch_status='O').values('booking_id', 'booking_no').distinct()
        sample_type_list        = FirstLevelMaster.objects.filter(master_type__code='CM6', is_active=True)
        merchant_name_list      = FinishEntryMt.objects.filter(is_active=True).values('id', 'booking_no')
        ...
        if bid:
            delete_unused_finish_temp_trans(bid)
            ...
            cutting_trans_list      = FinishTransactionDt.objects.filter(booking_id=bid).values('id', 'transaction_no')
        ...
        return render(request, 'finishing_transaction_entry.html', context)
    ...
    def post(self, request):
        ...
        finish_type = request.POST.get('finish_type')
        finish_qty = request.POST.get('finish_qty_val')
        size_breakup = request.POST.get('size_breakup')
        remarks = request.POST.get('remarks')
        qa_name = request.POST.get('qa_name') 
        return_date = request.POST.get('return_date') 
        qa_status = request.POST.get('qa_status') 
        ...
        transaction_no = get_next_number(FinishTransactionDt, 'SSMP')

        try:
            finish_dt = FinishTransactionDt.objects.create(
            ...
            try:
                print(finish_dt)
                cursor =  connections['default'].cursor()
                sql_ins = f""" insert into finish_entry_dt(qty,size,booking_no,booking_id,color,is_active,updated_at,created_at,finish_dt,created_by) 
                (select qty,size,booking_no,booking_id,color,is_active,updated_at,created_at,{finish_dt.id} as finish_dt,created_by 
                from transaction_entry_mt where booking_id = {booking_id.id} and trans_type='finish' )  """
                # print(sql_ins)
                cursor.execute(sql_ins)
                cursor.close()
                
                cursor =  connections['default'].cursor()
                sql_del = f""" delete from transaction_entry_mt where booking_id = {booking_id.id} and trans_type='finish' """
            ...
            print(e)
            messages.error(request, "Data Not Saved")
        return redirect ('finishing_transaction_entry_page')