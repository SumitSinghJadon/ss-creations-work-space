...
from functions import delete_unused_finish_temp_defect_trans , get_trans_srno_data , get_trans_srno_data_disp
from django.contrib import messages
from django.db import connection,connections
from datetime import datetime , timedelta

class finishingDefectsView(View):
    def get(self, request):
        trans_id = request.GET.get('trans_id')
        delete_unused_finish_temp_defect_trans(trans_id)
        trans_srno_list = get_trans_srno_data(trans_id,'finish')
        trans_srno_list_disp = get_trans_srno_data_disp(trans_id,'finish')
        ...
        return render(request, 'finishing_defects.html',context)
    ...
    def post(self, request):
        ...
        try:
                ...
                for i in range(counter):
                    ...
                    if result:
                        try:
                            ...
                            if result:
                                sql_ins_size  = f"""
                                insert into finish_entry_size_dt(trans_id,qty,size,qid,color,result,defect_count,defect,is_active,updated_at,created_at) 
            ...
            cursor =  connections['default'].cursor()
            sql_ins = f""" insert into defect_finish_entry_dt(qty,is_active,updated_at,created_at,defect_type,trans_type,defect_id,trans_id,size,qid,color) 
            (select qty,is_active,updated_at,created_at,defect_type,trans_type,defect_id,trans_id,size,qid,color
            from defect_transaction_entry_mt where trans_id = {trans_id} and trans_type='finish' )  """
            # print(sql_ins)
            cursor.execute(sql_ins)
            cursor.close()
            
            cursor =  connections['default'].cursor()
            sql_del = f""" delete from defect_transaction_entry_mt where trans_id = {trans_id} and trans_type='finish' """
