...
from Sampling_db.models import OtherTransMt, OtherSizeQuantity
...
class OtherTransView(View):
    def get(self, request):
        ...
        if flag == 'get_merchant_by_merchant_head':
            ...
            bid = request.GET.get("bid")
            OtherTransMt.objects.filter(id=bid).update(
                booking_status = 'cancelled'
            )
            return redirect ('other_entry_page')
        ...
        if bid:
            mt_data = OtherTransMt.objects.get(id=bid)
            dt_data = []
            sq_data = OtherSizeQuantity.objects.filter(booking_id=mt_data)

        merchant_head_list = CommonMaster.objects.filter(master_type__code='CT-22', is_active=True).using('intellisync_db')
        booking_no_list    = OtherTransMt.objects.filter(is_active=True).values('id', 'booking_no').order_by("-created_at")
        ...
        return render(request, 'other_entry.html', context)
    ...
    def post(self, request):
        ...
        if not bid:
            booking_no = get_next_number('OMPL')
        else:
            booking_no = OtherTransMt.objects.get(id=bid).booking_no
        ...
        if booking_no:
            ...
            create_data['image1'] = 'NA'
            create_data['image2'] = 'NA'

            # Check If Booking is already created with same data
            sample_data = OtherTransMt.objects.filter(
                buyer_code = buyer.party_code, 
                style_no = style_no, 
                season = season, 
                season_year = season_year, 
                # sample_group_type = sample_group_type_p, 
                sample_type = sample_type,
            ).first()
            ...
            if sample_data and not bid and sample_data.sample_group_type.value == 'P':
                ...
                if bid:
                    # Update Booking
                    booking_id = OtherTransMt.objects.get(id=bid)
                    ...
                    # Create New Booking
                    booking_id = OtherTransMt.objects.create(**create_data)
                ...
                if booking_id:
                    ...
                    OtherSizeQuantity.objects.filter(booking_no = booking_no).delete()
                    OtherSizeQuantity.objects.bulk_create(SSQ_list)
                    booking_id.save()

            #         except Exception as e:
            #             print(e)
            #             OtherSizeQuantity.objects.filter(booking_id=booking_id).delete()
            #             booking_id.delete()
