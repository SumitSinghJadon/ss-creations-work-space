...
const FileHandOver = () => {
                    ...
                    if ((data.pcd_status == "Meet" && data.reason_notmeet == null) || (data.pcd_status == "Not Meet" && data.reason_notmeet != null)) {
                        ...
                        if (
                            ...
                            if (row.original.handover_id != null) {
                                axios.put(`http://127.0.0.1:8000/FileHandOverViewSet/${row.original.handover_id}/`, data)
                                    .then(response => {
                                        alert("Data Updated Successfully")
                                    })
                            }
                            else {
                                axios.post('http://127.0.0.1:8000/FileHandOverViewSet/', data)
                                    .then(response => {
    ...
    useEffect(() => {
        if (selectedBuyer != null) {
            ...
            axios.get('http://127.0.0.1:8000/product/stylefilter/', {
                params: {
    ...
    useEffect(() => {
        axios.get('http://127.0.0.1:8000/product/filter/')
            .then(response => {
                const transformedOptions = response.data.filterdata.map(item => ({
